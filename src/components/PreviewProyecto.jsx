  ...
  return (
    <div className="border-r- border-b-3 border-l-3 border-slate-400 border-opacity-30 mt-2 shadow-lg mb-2 px-2">
      <div className="grid content-center h-16">
        <h2 className="text-xl md:text-xl">{nombre}</h2>
      </div>
      <div className="grid grid-cols-2">
        ...
        <div className="grid justify-items-end">
          <div className="d-inline">{cantidadColaboradores}</div>
        </div>
      </div>
      <div className="grid h-32 content-between">
        <p className="pt-2 text-lg text-gray-600">{contenido}</p>

        {auth._id !== creador ?
          <p className="bg-slate-700 font-bold rounded-lg uppercase p-1 text-sm text-white w-3/5 mb-1">Colaborador</p>
          : <p className="bg-green-600 font-bold rounded-lg uppercase p-1 text-smm text-white w-3/5 mb-1">Propietario</p>}
      </div>
      <hr className="border-slate-900 border-opacity-20 mt-0 mb-2 col-span-2"></hr>
      <div className="grid grid-cols-2 gap-2">
        <div className="px-1 text-base md:text-sm">Fecha inicio</div>
        <div className="px-1 text-base md:text-sm">Fecha entrega</div>
